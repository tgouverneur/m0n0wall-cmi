<?php
  ...
  */
  ...
  $head->set("subtitle", "Hardware information update");
  $head->set("pagename", "Hardware information update");
  ...
  if (!$ok) {
    ...
    $body->set("pagename", "Hardware information update");
    $body->set("action", "hwupdate.php");
    $main = Main::getInstance();
    $main->fetchMonoId();
    $main->fetchMonoDetails();
    $select = array(	"title" => "m0n0wall",
			"name" => "mid",
			"desc" => "Please select the m0n0wall you want to update hardware information"
		);
    $list = array();
  ...
  if ($ok) {
    ...
    foreach ($mono->hwInt as $eth) {
      $msg .= $eth->name."/".$eth->mac."<br/>\n";
      if (!$eth->existsDb()) {
        $eth->insert();
        $msg .= "    iface inserted in db<br/>";
      }
    }
  } else {
    $error = "Unable to fetch Hardware information: ".$error."<br/>";
  }
